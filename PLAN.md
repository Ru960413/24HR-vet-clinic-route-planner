...
- [x] 響應式檢查：表格改橫向捲動，手機版實測通過
- [x] details 頁改成由資料動態產生（Phase 3 一併完成）
- [x] 每間診所顯示「✓ 資料確認於 YYYY-MM」徽章
- [x] 「資訊有誤？回報」連結（連到站內 contact 表單）

## Phase 5：資料自動化維護

- [x] 一次性腳本：幫每間診所查出 Google `place_id`（49/49，散步的座標與 3 間搬家
      診所的地址已同步校正；候選新診所：永春〔信義〕、念念〔新莊〕待人工確認）
- [x] 排程比對腳本 `scripts/check_updates.py`：偵測歇業/搬家 + 各縣市發現掃描，
      GitHub Actions 每月 1 號跑，有發現就開 Issue（GitHub 自動寄 email 通知）
